...
# Import dataset and alternative model for upsampling.
from dataset import OxfordPetDataset
from strided_unet_upsample import StridedUNetUpsample
from losses import dice_loss  # Use the dice_loss function from losses.py
...
def main(args):
    ...
    # Initialize model
    try:
        print("[INFO] Initializing StridedUNetUpsample model...")
        model = StridedUNetUpsample(n_channels=3, n_classes=1).to(device)
    except Exception as e:
        print(f"[ERROR] Model initialization failed: {e}")
        return

    # Set loss function (Dice loss) and optimizer
    try:
        # We use our custom dice_loss function.
        def loss_fn(outputs, targets):
            # Apply sigmoid before calculating dice loss.
            return dice_loss(outputs, targets)
        optimizer = optim.Adam(model.parameters(), lr=args.lr)
        print("[INFO] Optimizer and Dice loss function set.")
    except Exception as e:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="StridedUNetUpsample (StrConv+Upsampling) with Dice Loss")
    ...
    parser.add_argument('--checkpoint_path', type=str, default="checkpoint_upsample.pth", help="Path to save the best model")
